...
rownames(expr1) = rownames(selected)
colnames(expr1) = "Expr1"
rownames(expr2) = rownames(selected)
colnames(expr2) = "Expr2"
colnames(FC) = "Fold Change"
rownames(FC) = rownames(selected)
colnames(logicalFC) = "Fold Change(log2 level)"
rownames(logicalFC) = rownames(selected)
#Compute Ave. Expr under control condition
for(gene in rownames(selected)){
  ...
  logFC = selected[gene,"logFC"]
  logicalFC[gene,"Fold Change(log2 level)"] = 2^logFC
  FC[gene,"Fold Change"] = Aval^(2^logFC-1)
}
summtab = data.frame(rownames(selected), FC, logicalFC, expr1, expr2, selected[,"t"], selected[,"P.Value"], selected[,"adj.P.Val"], selected[,"B"])
...
# Write gene names
write.table(rownames(selected), file=wpath_genenm_all, quote=F, row.names=F, col.names=F, sep="\t")
...
rownames(expr1) = rownames(selected2)
colnames(expr1) = "Expr1"
rownames(expr2) = rownames(selected2)
colnames(expr2) = "Expr2"
colnames(FC) = "Fold Change"
rownames(FC) = rownames(selected2)
colnames(logicalFC) = "Fold Change(log2 level)"
rownames(logicalFC) = rownames(selected2)
#Compute Ave. Expr under control condition
for(gene in rownames(selected2)){
  Avals = unlist(expr[gene, Ano])
  Aval = mean(Avals)
  Bvals = unlist(expr[gene, Bno])
  Bval = mean(Bvals)
  expr1[gene, 'Expr1'] = Aval
  expr2[gene, 'Expr2'] = Bval
  logFC = selected2[gene,"logFC"]
  logicalFC[gene,"Fold Change(log2 level)"] = 2^logFC
  FC[gene,"Fold Change"] = Aval^(2^logFC-1)
}
summtab = data.frame(rownames(selected2), FC, logicalFC, expr1, expr2, selected2[,"t"], selected2[,"P.Value"], selected2[,"adj.P.Val"], selected2[,"B"])
colnames(summtab) = c("ID", "Fold Change", "Fold Change(log2 level)", "Expr1", "Expr2", "t", "P.Value", "adj.P.Val", "B" )
write.table(summtab, file=wpath_info, quote=F, row.names=F, col.names=T, sep="\t")
# Write gene names
write.table(rownames(selected2), file=wpath_genenm, quote=F, row.names=F, col.names=F, sep="\t")
